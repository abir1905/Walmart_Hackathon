...
const ChatBot = () => {
  ...
  const addToCartAndReset = useCallback(() => {
    if (selectedProduct) {
      // Create the cart item
      const cartItem = {
        id: `${selectedProduct.id}-${selectedSize || 'na'}-${selectedColor || 'na'}`,
        name: selectedProduct.name,
        description: `${selectedSize ? `Size: ${selectedSize}` : ''} ${selectedColor ? `Color: ${selectedColor}` : ''} ${selectedQuantity > 1 ? `Qty: ${selectedQuantity}` : ''}`.trim(),
      ...
      // Add success message
      addBotMessage(`Added ${cartItem.name} to your cart! Would you like to add anything else?`);
    }
  ...
  }, [selectedProduct, selectedSize, selectedColor, selectedQuantity, addToCart, addBotMessage]);
    ...
    switch (activeStep) {
      case 0: // Initial state - product selection
        if (response.toLowerCase().includes('add') || 
            response.toLowerCase().includes('cart') || 
            response.toLowerCase().includes('want') ||
          ...
          if (matchedProduct) {
            ...
            // Determine next step based on product attributes
      ...
      case 1: // Size selection
        const sizeMatch = selectedProduct.sizes.find(size => 
          response.toLowerCase().includes(size.toLowerCase())
        );
        ...
        } else if (response.toLowerCase().includes('any') || 
                   response.toLowerCase().includes('random') ||
                   response.toLowerCase().includes('surprise')) {
      ...
      case 2: // Color selection
        const colorMatch = selectedProduct.colors.find(color => 
          response.toLowerCase().includes(color.toLowerCase())
        );
      ...
      case 3: // Quantity selection
        const quantityMatch = response.match(/\d+/);
        if (quantityMatch) {
          ...
          if (quantity > 0 && quantity <= 10) {
            ...
            setTimeout(addToCartAndReset, 1500);
          } else {
            addBotMessage("Please enter a quantity between 1 and 10.");
          }
        } else if (response.toLowerCase().includes('one') || 
                   response.toLowerCase().includes('1')) {
          setSelectedQuantity(1);
          setActiveStep(4);
          addBotMessage(`Adding one item to your cart.`);
          setTimeout(addToCartAndReset, 1500);
        } else {
          addBotMessage("I didn't catch the quantity. Could you specify a number?");
        }
        break;
  ...
  const stopListening = () => {
    ...
    try {
      // Stop only if we're currently listening
      if (isListeningRef.current) {
        recognitionRef.current.stop();
      }
